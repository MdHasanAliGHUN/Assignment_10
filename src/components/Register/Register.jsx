...
import { FaGoogle, FaGithub, FaFacebook, FaRegEyeSlash, FaRegEye,} from "react-icons/fa";
...
const Register = () => {
  ...
  const {singUpWithEmail, handleGoogleLoginWithSocial} = useAuth();
  const navigate = useNavigate();

  const onSubmit = async (data) => {
    try{
     await singUpWithEmail(data.email, data.password);
      Swal.fire({
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100 p-4">
      <div className="w-full max-w-md p-8 space-y-6 bg-white shadow-md rounded-lg">
          ...
          <div>
            ...
            <div className=" relative">
              ...
              <div className="form-control">
                <label className="cursor-pointer justify-start gap-4 label mt-2">
                  <input
                    type="checkbox"
                    defaultChecked
                    className="checkbox checkbox-primary"
                  />
                  <span className="label-text italic">Accepts Our Term And Conditons</span>
                </label>
              </div>
              <button
        ...
        <p className="text-base text-center text-gray-600">
          ...
          <Link to="/login" className="text-blue-600 hover:underline italic">
            Login
          </Link>
        </p>
      </div>
